...
public:
	 ...
	 */
	Plaintext(ZZX mx = ZZX::zero(), long logp = 0, long logq = 0,
			long slots = 1, bool isComplex = true) :
			mx(mx), logp(logp), logq(logq), slots(slots), isComplex(isComplex) {
	}
	...
	Plaintext(const Plaintext &o) :
			mx(o.mx), logp(o.logp), logq(o.logq), slots(o.slots), isComplex(
					o.isComplex) {
	}
};
